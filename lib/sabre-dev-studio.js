  ...
  function SabreDevStudio(options) {
    ...
    function init(options) {
      var clientID = function() {
        var str = 'V1:' + that.config.user + ':' + that.config.group + ':' + that.config.domain;
        return new Buffer(str).toString('base64');
      }
      var clientSecret = function() {
        return new Buffer(that.config.password).toString('base64')
      }
      var credentials = function() {
        return new Buffer(clientID() + ':' + clientSecret()).toString('base64');
      }
      var keys = ['user', 'group', 'domain', 'password', 'uri', 'access_token'];
